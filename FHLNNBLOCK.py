...
from criminal_nets_reader import get_criminal_net_edges_split, get_criminal_net_adj_split


class BlockNet(torch.nn.Module):
    def __init__(self,data,num_features,num_classes,boundary_matrics,dimension=128):
        super(BlockNet, self).__init__()
        self.conv1 = GCNConv(num_features, 64, cached=True)
        self.conv2 = GCNConv(64, 32, cached=True)
        self.boundary_matrics = boundary_matrics # [L1_lower, L1_upper], for both of them, shape is |E| * |E|
        self.leakyrelu = torch.nn.LeakyReLU(0.1, True) # 0.1 is better than 0.2
        ...
        self.embeddings_sim = nn.Parameter(torch.FloatTensor(data.x.size(0), int(boundary_matrix_size * 2)))
    ...
    def g_encode(self,data):
        ...
        x = F.dropout(x, p=0.5, training=self.training)
        x = F.relu(self.conv2(x, edge_index)) # shape of x is |V| * hidden_dim
        return x

    def s_encode(self,data,g_emb,type="train"): # total_edges information are defined in below call function; emb is from encode above function
        if type == 'train':
            edges_pos = data.total_edges[:data.train_pos] # total_edges 由 get_adj_split的所有outputs产生!
            index = np.random.randint(0, data.train_neg, data.train_pos) # 从train_neg中随机抽选出与train_pos数量一样的 fake edges!
        ...
        elif type == 'val': # for 'val', it contains val_pos and val_neg (即为非ground-truth的edges)
        ...
        elif type == 'test': # for 'test', it contains test_pos and test_neg (即为非ground-truth的edges)
        ...
        if zeros_off_diagonal:
            ...
            L0_r = torch.matrix_power(L0, 4)
            L1_r = torch.matrix_power(L1, 4)
            upper_block = torch.cat([L0_r, 0. * self.weights_off_diagonal * L0_r * L1_r ], dim=1)
            lower_block = torch.cat([torch.transpose(0. * self.weights_off_diagonal * L0_r * L1_r , 0, 1), L1_r], dim=1)
            sim_block = torch.cat([upper_block, lower_block], dim=0)  # 2|E| * 2|E|

        s_emb_sim_ = torch.matmul(self.embeddings_sim, sim_block) # |V| * 2|E|
        ...
        d_sim = (s_emb_sim_in - s_emb_sim_out).pow(2) #(1. - F.cosine_similarity(s_emb_sim0_in, s_emb_sim0_out)).view(-1,1)
        ...
        g_sqdist = (g_emb_in - g_emb_out).pow(2)#.sum(dim=-1)
...
def call(data,name,num_features,num_classes): # commented out data_cnt
    ...
    if name in ['PPI']:
        ...
        val_prop = 0.05
        test_prop = 0.1
    train_edges, train_edges_false, val_edges, val_edges_false, test_edges, test_edges_false = get_criminal_net_edges_split(data, val_prop = val_prop, test_prop = test_prop)
    ...
    if boundary_matrices_option:
        ...
        model, data = BlockNet(data, num_features, num_classes, boundary_matrics=[boundary_matrix0.to(device), boundary_matrix1.to(device)]).to(device), data.to(device)
        ...
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        data.total_edges_y.to(device)
        model, data = BlockNet(data, num_features, num_classes, boundary_matrics=[L0u.to(device), L1f.to(device)]).to(device), data.to(device)
