...
def get_adj_split(adj, val_prop=0.05, test_prop=0.1, seed=1234):
    ...
    #neg_edges = np.array(list(zip(x, y))) # shape of neg_edges is (3662708, 2)
    ...
    print("negative edges after shuffle: ", neg_edges.shape) # (194345579, 2)
...
# Cora-ML
#test_data = loaddatas('Cora')[0] # to access the output from loaddatas('name')[0]
#print(test_data.edge_index.size(1)/2)
# test_data.edge_index: shape is [2, 10556]; test_data.x; test_data.y
# a,b,c,d,e,f = get_edges_split(test_data, val_prop=0.05, test_prop = 0.1, seed= 123)
# shape of a: (4488,2)
# shape of b: (3663498, 2) # don't know why need this
# shape of c: (263, 2)
# shape of d: (263, 2)
# shape of e: (527, 2)
# shape of f: (527, 2)
...
# B1, B2 = compute_incidence_matrix(test_data) # shape of B1 is (2708, 5278), shape of B2 is (5278, 1630)

# PubMed
#test_data = loaddatas('PubMed')[0] # (test_data.edge_index).size() is (2, 88648)
#L1_lower, L1_upper = compute_hodge_matrix(test_data)
#np.savez_compressed('PubMed_L1_lower.npz', L1_lower) # (44324, 44324)
#np.savez_compressed('PubMed_L1_upper.npz', L1_upper) # (44324, 44324)
